...
async function writePages(element, path, schemaPath, dataPath, helperPath, moduleType, configPath) {
    ...
    const templateDirAttach = moduleType == "Javascript modules (import/export)" ? "lib/template/jsimport/pages_refactor_attach.dot" : "lib/template/commonjs/pages_refactor_attach.dot"
    ...
    if (element.request.hasOwnProperty('body')) {
        ...
        async getMappedBody(...args) {
            const defaultData = await new request_helper().getDefaultData(data.${name.replace('-', '_')}_data)
    ...
    if (element.request.hasOwnProperty('body')) { 
        if (dataPath.replace(/\\/g, "/").split('/').length >= 2) {
            code = code.replace("{{path_data}}", "\n const data = require('" + dataPath.replace(/\\/g, "/").split('/').slice(0, 2).join('/') + `/${toLowerCase(dataPath.replace(/\\/g, "/").split('/')[1])}` + ".data.js')")
        } else {
            code = code.replace("{{path_data}}", "\n const data = require('" + dataPath.replace(/\\/g, "/") + `/${name}` + ".data.js')")
        }
    } else {
        code = code.replace("{{path_data}}", '')
    }
