...
async function writeHelper() {
    ...
    const requestHelperDir = 'tests/helper';
    fs.mkdirSync(requestHelperDir, { recursive: true })
    ...
    await isFileExisted('tests', 'request.helper.js', function(data) {
        if (!data[0] && data[1] == 'tests/helper') {
            fs.writeFile('tests/helper/request.helper.js',
                fs.readFileSync('lib/template/requestHelper.dot', 'utf8') , function (err) { if (err) throw err ; });
        }  
    })
}
