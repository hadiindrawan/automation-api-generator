...
async function writeTest(element, path, pagesPath, dataPath, moduleType, configPath) {
    ...
    let suiteDataPath = toCamelCase(dataPath.includes('\\') ? dataPath.split('\\')[1] : element.name)
    ...
    code = code.replace("{{data_path}}", '@data/' + suiteDataPath + '.data.js')
    ...
    if (element.request.hasOwnProperty('url')) {
        try {
            ...
            if (!fileExists) {
                // create file test
                writeFile(path + '/' + method + '_' + name + '.spec.js', code, function (err) {
                    if (err) throw err;
                });
                await waitFor(500)
