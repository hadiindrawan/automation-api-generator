...
async function writeTest(element, path, pagesPath) {
    let contents = fs.readFileSync('./node_modules/dot-generator-mocha/template/test.dot', 'utf8');
...
async function writePages(element, path, jsonSchemaRelativePath, helperPath) {
    let contents = fs.readFileSync('./node_modules/dot-generator-mocha/template/request.dot', 'utf8');
    ...
    if (element.request.hasOwnProperty('body')) {
        ...
        if (element.request.body?.mode == 'formdata') {
            ...
            if (data.some(datas => datas['type'] === 'file')) {
                contents = fs.readFileSync('./node_modules/dot-generator-mocha/template/requestWithAttach.dot', 'utf8');
                ...
                contents = fs.readFileSync('./node_modules/dot-generator-mocha/template/request.dot', 'utf8');
                asyncForEach(data, async (body) => {
                    if (body.disabled != true) {
                        if (first === false) bodyRaw += ','+'\r\n'+'\t\t\t';
                        bodyRaw += '"' + body.key+'"'  + ': ' + '"'+ body.value +'"';
...
async function writeJsonSchema(element, jsonSchemaPath) {
    ...
    fs.writeFile(jsonSchemaPath + '/' +method+'_'+ name + '.json',
            fs.readFileSync('./node_modules/dot-generator-mocha/template/json_responses.dot', 'utf8') , function (err) { if (err) throw err ; });
}
...
async function generate() {
    fs.readFile(process.argv.slice(2)[0], (err, data) => {
        ...
        fs.writeFile('tests/helper/request.helper.js',
            fs.readFileSync('./node_modules/dot-generator-mocha/template/requestHelper.dot', 'utf8') , function (err) { if (err) throw err ; });
